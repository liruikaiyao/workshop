...
user_list = []
for elem in hyd_events.find({'Event':'unsubscribe'}):
...
for elem in user_list:
    user_all=[]
    curt = hyd_events.find({'$and':[{'FromUserName':elem},{ 'Event': { '$in': ['subscribe', 'unsubscribe']}}]})
    cur = hyd_events.find({'$and':[{'FromUserName':elem},{ 'Event': 'subscribe'}]}).count()
    if cur>=2:
        for item in curt:
